...
public class Apple extends DynamicEntity {
    ...
    private void wallBounce(Grid grid, Vector2 me, Vector2 vel) {
        Vector2 m1, m2;
        m1 = me.cpy().sub(getBounds().cpy().scl(.5f)).scl(1f / grid.getUnitSize()).add(.5f, .5f);
        m2 = me.cpy().add(getBounds().cpy().scl(.5f)).scl(1f / grid.getUnitSize()).add(.5f, .5f);
